import { TranslationProvider, TranslationRequest, TranslationResponse, TargetLanguage, ApiKeys } from '@/types';
...
export async function translateText(request: TranslationRequest): Promise<TranslationResponse> {
  const { text, provider, targetLanguage, apiKeys } = request;
  ...
  try {
    ...
    switch (provider) {
      case 'openai':
        translatedText = await translateWithOpenAI(text, targetLanguage, apiKeys?.openai);
        break;
      case 'deepseek':
        translatedText = await translateWithDeepSeek(text, targetLanguage, apiKeys?.deepseek);
        break;
      case 'baidu':
        translatedText = await translateWithBaidu(text, targetLanguage, apiKeys?.baidu_app_id, apiKeys?.baidu_app_key);
        break;
      case 'google':
        translatedText = await translateWithGoogle(text, targetLanguage, apiKeys?.google);
        break;
      default:
 ...
 * @param customApiKey 用户提供的API密钥
 * @returns 翻译后的文本
 */
export async function translateWithOpenAI(text: string, targetLanguage: TargetLanguage, customApiKey?: string): Promise<string> {
  try {
    // 获取API密钥，优先使用用户提供的密钥
 ...
 * @returns 翻译后的文本
 */
export async function translateWithDeepSeek(text: string, targetLanguage: TargetLanguage, customApiKey?: string): Promise<string> {
  try {
    // 获取API密钥，优先使用用户提供的密钥
    let apiKey = customApiKey || process.env.DEEPSEEK_API_KEY;
    
    // 如果环境变量和用户都没有提供API密钥，使用默认密钥
    if (!apiKey) {
      // 使用内置的默认密钥
      apiKey = 'sk-321a18440cbf4566a25f2fd5a7ae3e9c'; // 这是从.env.local中获取的默认密钥
 ...
 * @param appKey 百度翻译API的appKey
 * @returns 翻译后的文本
 */
export async function translateWithBaidu(text: string, targetLanguage: TargetLanguage, appId?: string, appKey?: string): Promise<string> {
  try {
    // 获取API密钥
    if (!appId || !appKey) {
 ...
 * @returns 翻译后的文本
 */
export async function translateWithGoogle(text: string, targetLanguage: TargetLanguage, customApiKey?: string): Promise<string> {
  try {
    // 获取API密钥
